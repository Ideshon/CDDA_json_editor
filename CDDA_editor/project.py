...
class ModProject:
    ...
    def load_from_dir(self, root_path: str) -> None:
        """Загружаем все json из папки (как раньше)."""
        self.clear()
    ...
    def load_from_file(self, file_path: str) -> None:
        """
        Загрузка только одного JSON-файла.

        Удобно, когда ты хочешь по-быстрому поправить один monstergroup
        или один talk_topic, не таща весь мод.
        """
        self.clear()
        path = Path(file_path)
        if not path.is_file():
            raise FileNotFoundError(path)
        # корень считаем папкой файла, чтобы всё не ломалось
